  ...
  return (
    <SafeAreaView edges={["top", "left", "right", "bottom"]} style={styles.screen}>
      <View style={styles.header}>
        <Pressable onPress={goBack} hitSlop={8} style={({ pressed }) => [styles.circleButton, pressed && styles.pressed]}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <SettingsCard>
          ...
          <Divider />
          <SettingsRow icon="terminal" title={zh ? "Codex 运行环境" : "Codex Runtime"} subtitle={zh ? "模型、权限与工作区" : "Model, permissions and workspace"} onPress={openDetailedSettings} />
        </SettingsCard>

        <SectionTitle>{zh ? "通知与状态" : "Notifications & Status"}</SectionTitle>
        <SettingsCard>
          <SettingsRow icon="running" title={zh ? "通知" : "Notifications"} subtitle={zh ? "任务完成与需要操作时提醒" : "Turn completion and action-required alerts"} onPress={openDetailedSettings} />
          <Divider />
          <SettingsRow icon="goal" title={zh ? "用量限制" : "Usage Limits"} subtitle={zh ? "查看当前 Codex 使用额度" : "View current Codex usage"} onPress={openDetailedSettings} />
        </SettingsCard>

        <SectionTitle>{zh ? "应用" : "App"}</SectionTitle>
        <SettingsCard>
          <SettingsRow icon="refresh" title={zh ? "更新" : "Updates"} subtitle={zh ? "检查应用与热更新" : "Check app and hot updates"} onPress={openDetailedSettings} />
          <Divider />
          <SettingsRow icon="controls" title={zh ? "高级设置" : "Advanced Settings"} subtitle={zh ? "服务器、调试与完整设置" : "Server, diagnostics and full settings"} onPress={openDetailedSettings} />
        </SettingsCard>
  ...
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.row, pressed && styles.rowPressed]}>
      ...
      <View style={styles.rowCopy}>
        ...
        <ThemedText numberOfLines={1} style={styles.rowSubtitle}>{subtitle}</ThemedText>
      </View>
...
const styles = StyleSheet.create({
  ...
  sectionTitle: { color: "#8E8E93", fontSize: 15, lineHeight: 20, fontWeight: "600", marginTop: 22, marginBottom: 9, marginLeft: 4 },
  card: { backgroundColor: "#1C1C1E", borderRadius: 18, overflow: "hidden" },
  row: { minHeight: 68, paddingHorizontal: 15, flexDirection: "row", alignItems: "center", gap: 12 },
  rowPressed: { backgroundColor: "rgba(255,255,255,0.06)" },
  rowIcon: { width: 36, height: 36, borderRadius: 10, backgroundColor: "#252529", alignItems: "center", justifyContent: "center" },
  ...
  divider: { height: StyleSheet.hairlineWidth, backgroundColor: "rgba(255,255,255,0.09)", marginLeft: 63 },
